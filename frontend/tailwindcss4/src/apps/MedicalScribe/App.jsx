// apps/MedicalScribe/App.jsx (Updated)
...
const getFriendlySpeakerLabel = (speakerId, speakerRoles) => {
  ...
  const speakerNum = parseInt(speakerId.replace('spk_', ''), 10);
...
const HighlightedText = React.memo(({ text, entities }) => {
  ...
  const nonDominatedEntities = entities.filter(entityA => 
    !entities.some(entityB => 
      entityA !== entityB &&
      entityA.BeginOffset >= entityB.BeginOffset &&
      entityA.EndOffset <= entityB.EndOffset &&
      (entityB.BeginOffset < entityA.BeginOffset || entityB.EndOffset > entityA.EndOffset)
    )
  );
  ...
  const sortedEntities = finalEntities.sort((a, b) => a.BeginOffset - b.BeginOffset);
  ...
  sortedEntities.forEach((entity, index) => {
    if (entity.BeginOffset > lastIndex) {
      parts.push(<span key={`text-${lastIndex}`}>{text.substring(lastIndex, entity.BeginOffset)}</span>);
    }
    const tooltipText = entity.Category.replace(/_/g, ' ');
    parts.push(
      <span key={`entity-${index}`} className={`entity entity-${entity.Category}`} data-tooltip={tooltipText}>
...
const TranscriptSegment = React.memo(({ segment, speakerRoles, onSpeakerRoleToggle }) => (
  <p>
    <strong className="speaker-label-clickable" onClick={() => onSpeakerRoleToggle(segment.speaker)} title="Click to change role">
      [{getFriendlySpeakerLabel(segment.speaker, speakerRoles)}]:
    </strong>
    {' '}
...
const EditPencilIcon = () => (
  <svg className="edit-pencil-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" width="16" height="16">
...
const NotesPanel = ({ notes, setNotes, loading, error }) => {
  ...
  const handleCancel = () => { setEditingKey(null); setEditText(""); };

  const handleSave = () => {
    if (editingKey === null) return;
    const originalItems = notes[editingKey];
    let newItems;
    if (Array.isArray(originalItems) && originalItems.every(i => typeof i === 'object' && 'text' in i)) {
      newItems = editText.split('\n').filter(line => line.trim() !== '').map(line => ({ text: line.trim() }));
    } else {
      newItems = editText;
    }
  ...
  const handleCopy = () => {
    if (!notes) return;
    const noteString = Object.entries(notes).map(([section, items]) => {
  ...
  const renderContent = () => {
    if (loading) return <div className="spinner-container"><div className="spinner"></div><p>Generating initial note...</p></div>;
      ...
      return (
        <ul className="notes-list">
          {Object.entries(notes).map(([section, items]) => {
            ...
            const isAssessmentPlan = section === 'Assessment and Plan' && typeof items === 'string';
              ...
              <li key={section} className="notes-list-item">
                ...
                {isEditing ? (
                  <div className="note-edit-container">
                    <textarea className="note-editor" value={editText} onChange={(e) => setEditText(e.target.value)} rows={Array.isArray(items) ? Math.max(3, items.length) : 8} autoFocus />
                  ...
                  <div className="editable-section" onClick={() => handleEditClick(section, items)}>
                    <EditPencilIcon />
                    {isNarrative && items === "None" ? <p className="narrative-text none-text">{items}</p>
                    : isNarrative && isAssessmentPlan ? <div className="assessment-plan">{items.split('\n').map((line, index) => line.trim() ? <p key={index}>{line.trim()}</p> : null)}</div>
                    : isNarrative ? <p className="narrative-text">{items}</p>
                    : <ul className="notes-sublist">{items.map((item, index) => <li key={index}>{item.text}</li>)}</ul>}
                  </div>
                )}
              </li>
            );
    ...
    return <p className="placeholder-text">Click "Start Session" to begin.</p>;
  };
  ...
  return (
    <div className="panel">
      <div className="panel-header">
        ...
        {notes && <button onClick={handleCopy} className="button button-copy">{copied ? 'Copied!' : 'Copy'}</button>}
      </div>
...
export default function MedicalScribeApp() {
  ...
  useEffect(() => { transcriptEndRef.current?.scrollIntoView({ behavior: "smooth" }); }, [transcriptSegments, interimTranscript]);
  ...
  const startSession = async () => {
    ...
    try {
      ...
      ws.onopen = () => { setConnectionStatus('connected'); startMicrophone(); };
      ws.onmessage = (event) => {
        try {
          const data = JSON.parse(event.data);
          if (!data.Transcript?.Results?.length || !data.Transcript.Results[0].Alternatives?.length) return;
          const result = data.Transcript.Results[0];
          const alternative = result.Alternatives[0];
          const transcriptText = alternative.Transcript;
          const firstWord = alternative.Items?.find(item => item.Type === 'pronunciation');
          const currentSpeaker = firstWord ? firstWord.Speaker : null;
          if (result.IsPartial) {
            setInterimTranscript(transcriptText);
            setInterimSpeaker(currentSpeaker);
          } else {
            if (currentSpeaker && !hasShownHintRef.current) { setShowSpeakerHint(true); hasShownHintRef.current = true; }
            const finalSegment = { id: result.ResultId, speaker: currentSpeaker, text: transcriptText, entities: data.ComprehendEntities || [] };
            setTranscriptSegments(prev => new Map(prev).set(result.ResultId, finalSegment));
            setInterimTranscript("");
            setInterimSpeaker(null);
          }
        } catch (e) { console.error("Error processing transcript message:", e); }
      };
      ws.onerror = (err) => {
  ...
  const startMicrophone = async () => {
    try {
      ...
      workletNode.port.onmessage = (event) => {
        if (sessionStateRef.current === 'recording' && websocketRef.current?.readyState === WebSocket.OPEN) {
  ...
  const stopSession = (closeSocket = true) => {
    ...
    if (closeSocket && websocketRef.current?.readyState === WebSocket.OPEN) websocketRef.current?.close();
  };
  ...
  const handleGenerateNote = async () => {
    ...
    Array.from(transcriptSegments.values()).forEach(seg => {
      transcriptWithRoles += `[${getFriendlySpeakerLabel(seg.speaker, speakerRoles)}]: ${seg.text}\n`;
    });
  ...
  const getStatusDisplay = () => {
    if (connectionStatus === 'error') return <span className="status-text status-error">Connection Error</span>;
    switch (sessionState) {
      case 'recording': return <><div className="recording-indicator"></div> <span className="status-text">Recording</span></>;
      case 'paused': return <><div className="recording-indicator paused"></div> <span className="status-text">Paused</span></>;
      case 'stopped': return <span className="status-text">Session Ended</span>;
      case 'connecting': return <span className="status-text">Connecting...</span>;
      default: return <span className="status-text">Ready</span>;
    }
  };
  ...
  const renderActionButtons = () => {
    const primaryButton = () => {
      switch (sessionState) {
        case 'idle':
        case 'stopped':
          return <button onClick={startSession} className="button button-primary">{sessionState === 'idle' ? 'Start Session' : 'New Session'}</button>;
        case 'recording':
          return <button onClick={handlePause} className="button button-primary">Pause</button>;
        case 'paused':
          return <button onClick={handleResume} className="button button-primary">Resume</button>;
        case 'connecting':
          return <button className="button button-primary" disabled>Connecting...</button>;
        default: return null;
      }
    };
    return (
      <div className="action-buttons">
        ...
        {(sessionState === 'recording' || sessionState === 'paused') && (
          <button onClick={() => stopSession()} className="button button-secondary">Stop Session</button>
        )}
        {sessionState === 'stopped' && (
          <button onClick={handleGenerateNote} className="button button-generate" disabled={loading}>
  ...
  return (
    <div className="app-container">
      <header className="app-header">
        <img src="/stethoscribe.png" alt="StethoscribeAI Logo" className="logo-image" />
      ...
      <main className="content-container">
        <div className="panel">
          <div className="panel-header">
            ...
            {showSpeakerHint && <div className="speaker-hint">Tip: Click a speaker label to assign a role.</div>}
          </div>
          <div className="transcript-box">
            {transcriptSegments.size > 0 || ['recording', 'paused'].includes(sessionState) ? (
              <>
                {Array.from(transcriptSegments.values()).map((seg) => (
                  <TranscriptSegment key={seg.id} segment={seg} speakerRoles={speakerRoles} onSpeakerRoleToggle={handleSpeakerRoleToggle} />
                ))}
                {interimTranscript && <p className="interim-transcript">[{getFriendlySpeakerLabel(interimSpeaker, speakerRoles)}]: {interimTranscript}</p>}
                <div ref={transcriptEndRef} />
              </>
            ) : (
              <p className="placeholder-text">Transcript will appear here once a session starts.</p>
            )}
          </div>
        </div>
