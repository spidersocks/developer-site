...
// --- CHANGED: Define the new API base URL ---
...
// All helper functions (formatTopicLabel, getRgba, AxisLabel) remain unchanged
...
function AxisLabel() {
  return (
    <div style={{ width: "100%", display: "flex", justifyContent: "center", alignItems: "center", margin: "0px 0 0px 0" }}>
      <svg width="530" height="36" style={{ maxWidth: "100%", height: "36px" }} viewBox="0 0 530 36" aria-label="Relative Stance Axis" preserveAspectRatio="xMidYMid meet">
        <text x="0" y="27" fontSize="0.85em" fill="#64748b" fontWeight="500" textAnchor="start">less favorable</text>
        <line x1="115" y1="18" x2="185" y2="18" stroke="#64748b" strokeWidth="2" markerStart="url(#arrowleft)" />
        <text x="260" y="27" fontSize="0.95em" fill="#29292a" fontWeight="700" textAnchor="middle" letterSpacing="0.04em">Relative Stance</text>
        <line x1="335" y1="18" x2="405" y2="18" stroke="#64748b" strokeWidth="2" markerEnd="url(#arrowright)" />
        <text x="530" y="27" fontSize="0.85em" fill="#64748b" fontWeight="500" textAnchor="end">more favorable</text>
        <defs>
          <marker id="arrowleft" markerWidth="5" markerHeight="5" refX="4" refY="2.5" orient="auto"><polygon points="5,0 1,2.5 5,5" fill="#64748b" /></marker>
          <marker id="arrowright" markerWidth="5" markerHeight="5" refX="1" refY="2.5" orient="auto"><polygon points="0,0 4,2.5 0,5" fill="#64748b" /></marker>
        </defs>
      </svg>
    </div>
  );
...
export default function StanceChart() {
  ...
  useEffect(() => {
    setLoading(true);
    const params = selectedTopics.length ? "?topics=" + encodeURIComponent(selectedTopics.join(",")) : "";
    
    // --- CHANGED: Fetch from the new unified backend URL ---
    fetch(`${API_BASE}/api/stance/zscores/${params}`)
      .then(res => res.json())
  ...
  // All hooks and rendering logic below remain unchanged
  ...
  const sortedTopics = useMemo(() => {
    return topicAgg
      .map(([t, v]) => ({ topic: t, news: v.news, podcast: v.podcast, diff: v.podcast - v.news }))
  ...
  const lines = sortedTopics.map(row => {
    ...
    const color = row.diff > 0 ? getRgba(PODCAST_COLOR, intensity) : getRgba(NEWS_COLOR, intensity);
    return { x: [row.news, row.podcast], y: [formatTopicLabel(row.topic), formatTopicLabel(row.topic)], mode: "lines", line: { color, width: 6 }, hoverinfo: "skip", showlegend: false };
  });
  ...
  const newsTrace = {
    ...
    marker: { color: "#6366f1", symbol: "square", size: 12 },
  ...
  const podcastTrace = {
    ...
    y: sortedTopics.map(row => formatTopicLabel(row.topic)),
    mode: "markers", name: "Podcast",
    marker: { color: "#fb923c", symbol: "circle", size: 12 },
        ...
        {loading ? (
          ...
          <Plot
            ...
            layout={{
                ...
                yaxis: {
                    title: "", showticklabels: true, automargin: true,
                ...
                shapes: [{
                    type: 'line', x0: 0, x1: 0, y0: -0.5, y1: sortedTopics.length - 0.5,
