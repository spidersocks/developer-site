...
// List of drillable topics (unchanged)
...
function toTitleCase(str) {
  ...
  return str.replace(/\w\S*/g, function(txt, i, full) {
    if (i !== 0 && i + txt.length !== full.length && txt.match(smallWords)) return txt.toLowerCase();
    return txt.charAt(0).toUpperCase() + txt.substr(1).toLowerCase();
  });
}

function generateChartTitle(drillPath) {
  if (!drillPath || drillPath.length === 0) return "Distribution of News Topics by Source";
  if (drillPath.length  >= 1) return `Distribution of ${toTitleCase(drillPath[drillPath.length - 1])} Topics by Source`;
}
...
export default function PodcastChart() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const pathParam = drillPath.length ? "?path=" + encodeURIComponent(drillPath.join(PATH_SEPARATOR)) : "";
      
      // --- CHANGED: Fetch from the correct unified backend URL ---
      const endpoint = drillPath.length > 0
        ? `${API_BASE}/api/topics/drilldown/${pathParam}`
        : `${API_BASE}/api/topics/`; // Use the root endpoint for the initial view
  ...
  const topicOptions = useMemo(() => Array.from(new Set(topics.map((t) => t.label).filter(Boolean))), [topics]);
  const drillableOptions = useMemo(() => topicOptions.filter((label) => DRILLABLE_TOPICS.includes(label)), [topicOptions]);
  const handleDrilldown = (nextLabel) => setDrillPath([...drillPath, nextLabel]);
  const handleBreadcrumb = (idx) => setDrillPath(drillPath.slice(0, idx + 1));
  const handleBarClick = (data) => {
    if (data && DRILLABLE_TOPICS.includes(data.label)) {
      setDrillPath([...drillPath, data.label]);
    }
  };
  const chartData = useMemo(() => groupDataByMedium(topics), [topics]);
  const sourceTypes = useMemo(() => Array.from(new Set(topics.map((t) => t.source_type))), [topics]);
  const formattedLabels = useMemo(() => chartData.map(item => firstTwoWordsAndMore(item.label, 17)), [chartData]);
  const maxLabelWidth = useMemo(() => {
    if (typeof window === "undefined" || formattedLabels.length === 0) return 90;
    return Math.max(...formattedLabels.map(label => getTextWidth(label, "12px Arial")));
  }, [formattedLabels]);
  const rotatedLabelHeight = useMemo(() => Math.ceil(maxLabelWidth * 0.7071 + 12 * 0.7071) + 8, [maxLabelWidth]);
  ...
  return (
    <div>
      <nav className={styles.breadcrumbs} aria-label="Breadcrumb">
        <span className={styles.allTopicsLink} onClick={() => setDrillPath([])}>
          {drillPath.length === 0 ? "All Topics" : "Return to all topics"}
        </span>
        {drillPath.map((step, i) => (
          <React.Fragment key={i}>
            <span className={styles.sep}>&rsaquo;</span>
            <span style={{ color: i === drillPath.length - 1 ? "#3730a3" : "#6366f1", fontWeight: i === drillPath.length - 1 ? 600 : 500, cursor: "pointer" }} onClick={() => handleBreadcrumb(i)}>{step}</span>
          </React.Fragment>
        ))}
      </nav>
      {drillableOptions.length > 0 && (
        <div style={{ marginBottom: "1.5em" }}>
          ...
          <select aria-label="Drilldown topic selection" className={styles.dropdown} value="" onChange={(e) => { if (e.target.value) handleDrilldown(e.target.value); }}>
            <option value="">-- Select --</option>
            {drillableOptions.map((label) => (<option key={label} value={label}>{label}</option>))}
          </select>
        </div>
      )}
      <h3 className={styles.chartTitle} style={{ marginTop: 0, marginBottom: "1em" }}>{generateChartTitle(drillPath)}</h3>
      <div className={styles.chartScrollWrapper}>
        <div className={styles.chartInner} style={{ "--bar-count": chartData.length, minWidth: 500, width: Math.max(500, chartData.length * 80), maxWidth: "100%" }}>
          {loading ? (
            <div><div className={styles.spinner} /><div style={{ textAlign: "center", color: "#64748b" }}>Loading chart…</div></div>
          ) : chartData.length === 0 ? (
            <div style={{ textAlign: "center", color: "#64748b", padding: "2em" }}>No subtopics found.<br /><button onClick={() => setDrillPath(drillPath.slice(0, -1))} className={styles.button} style={{ marginTop: 16 }}>Go Back</button></div>
          ) : (
            <ResponsiveContainer width="100%" height={400}>
              <BarChart data={chartData} barCategoryGap="15%" barGap={2} margin={{ top: 40, right: 32, left: leftMargin, bottom: Math.max(105, bottomMargin) }}>
                <Legend verticalAlign="top" align="right" wrapperStyle={{ top: 0 }} />
                <XAxis dataKey="label" interval={0} tickLine={false} axisLine={false} type="category"
                  tick={({ x, y, payload }) => {
                    const label = firstTwoWordsAndMore(payload.value, 17);
                    return (<text x={x} y={y} dy={20} textAnchor="end" fill="#64748b" fontSize={13} style={{ fontWeight: 500, cursor: DRILLABLE_TOPICS.includes(payload.value) ? "pointer" : "default" }} transform={`rotate(-45, ${x}, ${y + 16})`}><title>{payload.value}</title>{label}</text>);
                  }}
                  label={{ value: `${toTitleCase(broaderTopic)} subtopics`, position: "bottom", offset: 90, fontSize: axisFontSize, fontWeight: 600, fill: "#29292a" }}
                />
                <YAxis axisLine={false} tickLine={false} domain={[0, "auto"]}
                  tick={({ x, y, payload }) => (<text x={x - 8} y={y + 4} textAnchor="end" fontSize={13} fill="#64748b">{(payload.value * 100).toFixed(0)}%</text>)}
                  width={60}
                  label={{ value: `Proportion of topics`, angle: -90, position: "insideLeft", dx: -20, fontSize: axisFontSize, fontWeight: 600, fill: "#29292a" }}
                />
                <Tooltip content={<CustomTooltip />} offset={35} />
                {sourceTypes.map((src) => (
                  <Bar key={src} dataKey={src} fill={SOURCE_TYPE_COLORS[src] || DEFAULT_BAR_COLOR} onClick={handleBarClick} cursor={({ payload }) => DRILLABLE_TOPICS.includes(payload.label) ? "pointer" : "default"} isAnimationActive={true} animationDuration={900} animationEasing="ease-out" />
                ))}
              </BarChart>
            </ResponsiveContainer>
          )}
